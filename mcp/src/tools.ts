...
const getPriceTrendsSchema = z.object({
  ...
  set_name: z.string().optional().describe("Optional set name"),
  condition: z
    .string()
    .optional()
...
const detectQuantitySpikesSchema = z.object({
  threshold: z
    .number()
    .default(1.5)
    .describe("Z-score threshold for spike detection"),
});

mcp.addTool({
  name: "get_price_trends",
  description: "Get price trends for a Pokemon card over time",
  parameters: getPriceTrendsSchema,
  execute: async (args: unknown, context: any) => {
    try {
      const parsed = getPriceTrendsSchema.parse(args);

      // Your database query logic here
      // Calculate start date based on days_back
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - parsed.days_back);

      // Generate cardId from card_name and set_name (adjust based on your logic)
      const cardId = parsed.set_name
        ? `${parsed.card_name}_${parsed.set_name}`
        : parsed.card_name;
      ...
      const averagePrice =
        trendData.reduce((sum, item) => sum + item.price, 0) / trendData.length;

      const result = {
        card_name: parsed.card_name,
        set_name: parsed.set_name,
        condition: parsed.condition,
        period_days: parsed.days_back,
...
mcp.addTool({
  ...
  parameters: detectQuantitySpikesSchema,
  execute: async (args: unknown, context: any) => {
    try {
      const parsed = detectQuantitySpikesSchema.parse(args);

      const spikes = await getQuantitySpikes(parsed.threshold);
