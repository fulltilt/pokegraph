...
function handleUnknownMethod(id: any, method: string) {
  return {
    jsonrpc: "2.0",
    id,
    error: {
...
function handleInternalError(id: any, error: any) {
  return {
    jsonrpc: "2.0",
    id,
    error: {
...
function zodToJsonSchema(zodSchema: any): any {
  ...
  if (zodSchema._def?.typeName === "ZodObject") {
    ...
    for (const [key, value] of Object.entries(zodSchema._def.shape())) {
      ...
      if (!field._def?.defaultValue && !field.isOptional?.()) {
        required.push(key);
      }
    }
...
async function handleMCPRequest(request: any): Promise<any> {
  try {
    switch (request.method) {
      ...
      default:
        return handleUnknownMethod(request.id, request.method);
    }
  } catch (error) {
    return handleInternalError(request.id, error);
  }
}
...
app.get("/", (req, res) => {
  res.json({
    message: "Poke-Trades MCP Server (Express)",
    endpoints: {
      health: "/health",
      mcp: "/mcp",
    },
...
export { app, startServer, handleMCPRequest };
...
// //         .number()
// //         .default(30)
// //         .describe("Period in days to analyze volatility"),
// //     }),
// //   },
...
// //         .number()
// //         .default(0.05)
// //         .describe("Sensitivity threshold for peak/valley detection"),
// //     }),
// //   },
...
// //           curr > prev &&
// //           curr > next &&
// //           (curr - Math.min(prev, next)) / curr > sensitivity
// //         ) {
// //           peaks.push({
...
// //           curr < prev &&
// //           curr < next &&
// //           (Math.max(prev, next) - curr) / Math.max(prev, next) > sensitivity
// //         ) {
// //           valleys.push({
// //             date: priceData[i].date.toISOString().split("T")[0],
// //             price: curr,
...
// //       if (peaks.length > 0) {
// //         const globalPeak = peaks.reduce((max, peak) =>
// //           peak.price > max.price ? peak : max
// //         );
// //         globalPeak.type = "global_peak";
// //       }

// //       if (valleys.length > 0) {
// //         const globalValley = valleys.reduce((min, valley) =>
// //           valley.price < min.price ? valley : min
// //         );
// //         globalValley.type = "global_valley";
// //       }

// //       const peakToValleyRatio =
// //         peaks.length > 0 && valleys.length > 0
// //           ? Math.max(...peaks.map((p) => p.price)) /
// //             Math.min(...valleys.map((v) => v.price))
// //           : 0;
...
// //     } catch (error) {
// //       throw new Error(
// //         `Failed to find peaks and valleys: ${
// //           error instanceof Error ? error.message : "Unknown error"
// //         }`
// //       );
// //     }
...
// //       condition: z.string().optional().describe("Card condition"),
// //       timeframe: z
// //         .enum(["1week", "1month", "3months", "1year"])
// //         .default("1month")
// //         .describe("Timeframe for growth calculation"),
// //     }),
// //   },
...
// // server.tool(
// //   "get_fair_value_estimate",
// //   {
// //     description:
// //       "Estimate fair value of a Pokemon card based on various factors",
// //     inputSchema: z.object({
// //       card_name: z.string().describe("Name of the Pokemon card"),
// //       set_name: z
// //         .string()
...
// //         condition === "mint" ? 1.2 : condition === "near_mint" ? 1.0 : 0.8;
...
// //         .number()
// //         .default(0.2)
// //         .describe("Target return percentage (default 20%)"),
// //     }),
// //   },
...
// // server.tool(
// //   "market_quantity_overview",
// //   {
// //     description: "Get a comprehensive overview of market quantity metrics",
// //     inputSchema: z.object({
// //       market_view: z
// //         .enum(["global", "regional", "platform-specific"])
// //         .default("global")
// //         .describe("Scope of market view"),
// //       aggregation_level: z
...
// //           market_cap_estimate: 1500000000.0,
// //         },
// //         top_categories: [
...
// //     } catch (error) {
// //       throw new Error(
// //         `Failed to generate market overview: ${
// //           error instanceof Error ? error.message : "Unknown error"
// //         }`
// //       );
// //     }
