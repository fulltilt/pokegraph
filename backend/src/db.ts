...
export const getCardPriceTrends = async (cardId: string, startDate: Date, endDate: Date) => {
...
export const getCardVolatility = async (cardId: string, date: Date) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, date: Date; price: number; daily_return: number }[]
  >(
    `
...
export const findPricePeaksAndValleys = async (cardId: string, date: Date) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, volatility: number; avg_return: number; data_points: number }[]
  >(
    `
      WITH daily_returns AS (
        SELECT 
...
export const getPriceGrowthRate = async (cardId: string, startDate: Date, endDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, start_price: Number, end_price: number, start_date: Date, end_date: Date, 
      growth_rate_pct: number; days_elapsed: number }[]
  >(
    `
...
export const compareCardPerformance = async (cardId: string, startDate: Date, endDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, card_name: String, min_price: number, max_price: number, start_price: number, 
      end_price: number, return_pct: number, volatility_range: number, data_points: number }[]
  >(
    `
...
export const rankCardsByROI = async (startDate: Date, limit: number) => {
  return await prisma.$queryRaw<
    { cardId: string, card_name: string, start_price: number, current_price: number, roi_pct: number }[]
  >`
    WITH card_roi AS (
      SELECT 
        pe."cardId",
...
export const findPriceCorrelations = async (startDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { card_a: string, card_b: string; correlation: number }[]
  >(
    `
      WITH daily_returns AS (
        SELECT 
...
export const findUndervaluedCards = async (startDate: Date, limit: number) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, card_name: string, set_name: string, avg_price: number; current_price: number,
      deviation_from_avg: number; upside_potential: number }[]
  >(
    `
      WITH price_stats AS (
        SELECT 
...
export const findBreakoutCandidates = async (limit: number) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, card_name: string; current_price: number; recent_ma_7d: number, recent_ma_30d: number,
...
export const findAnomalies = async (startDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, card_name: string, date: Date; price: number; prev_price: number,
      change_pct: number, z_score: number }[]
  >(
    `
...
export const analyzeSetPerformance = async (timeframe: '10d' | '1m' | '6m' | '1y', startDate: Date) => {
  return await prisma.$queryRaw`
    SELECT 
      c.data->>'set' as set_name,
...
export const marketConcentrationAnalysis = async (timeFrame: '10d' | '1m' | '6m' | '1y', startDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, date: Date; price: number; daily_return: number }[]
  >(
    `
...
export const getFairValueEstimate = async (cardId: string, startDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, current_price: number; mean_price: number, median_price: number, recent_avg: number,
      price_stddev: number, trend_direction: number, fair_value_estimate: number, valuation_signal: string,
      data_points: number }[]
  >(
    `
...
export const getBestBuyTiming = async (cardId: string, startDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { weekday: string, day_of_month: string; avg_daily_change: number; change_volatility: number,
      dip_opportunities: number, occurrences: number, timing_signal: string }[]
  >(
    `
...
export const calculateHoldingPeriods = async (cardId: string, startDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { holding_period: string, scenarios: string; avg_return_pct: number; volatility: number,
      win_rate_pct: number; max_gain_pct: number; max_loss_pct: number; risk_adjusted_return: number }[]
  >(
    `
...
export const getQuantitySpikes = async (startDate: Date, threshold: number = 1.5) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, card_name: string, set_name: String, date: Date; spike_quantity: number; normal_avg_quantity: number,
      z_score_30d: number; z_score_7d: number; pct_above_normal: number; spike_severity: string,
      spike_day_price: number; prev_day_price: number }[]
  >(
    `
      ...
      WHERE sd.z_score_30d >= $2  -- Configurable threshold (e.g., 1.5 for moderate spikes)
...
export const getQuantitySpikesWithPriceImpact = async (startDate: Date, threshold: number = 1.5) => {
  return await prisma.$queryRawUnsafe<
    { cardId: string, card_name: string, date: Date; spike_quantity: number; normal_quantity: number,
      spike_z_score: number; spike_price: number; prev_price: number; spike_type: string,
      price_change_same_day: number; price_change_next_day: number }[]
  >(
    `
...
export const topGainers = async (timeframe: '10d' | '1m' | '6m' | '1y', limit: number) => {
  return await prisma.cardPriceChangeSummary.findMany({
    where: {
...
export const analyzeTimeframePerformance = async (startDate: Date) => {
  return await prisma.$queryRawUnsafe<
    { timeframe: string, type: string; avg_change: number; volatility: number; card_count: number;
      max_change: number; min_change: number }[]
  >(
    `
      SELECT 
        cps.timeframe,
